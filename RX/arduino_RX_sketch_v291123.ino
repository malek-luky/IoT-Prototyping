...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(button), pushButton_ISR, CHANGE);  // use interrupt pin2 
...
void loop()
{
    ...
    if(buttonPushed = true) {    //button interrupt function
      if(digitalRead(button) == LOW & buttonprevState == HIGH & buttonnextState == HIGH) {
          noTone(buzzer);
          buz_on_hold = true;
          startMillis = millis();
          }
    ...
    if (currentMillis - startMillis >= period) { // check if 1 min passed --- !!!***regulable offset***!!!
             buz_on_hold = false;
    }
    buttonPushed = false;
